...
@dc.dataclass
class Station:
    _min: float = dc.field(default=float("+inf"), init=False)
    _max: float = dc.field(default=float("-inf"), init=False)
    _sum: float = dc.field(default=0, init=False)
    ...
    def get_mean(self) -> float:
        return java_round(self._sum * 10 / self._count) / 10
...
def solve(path: str) -> str:
    ...
    for line in data:
        ...
        stations[name].update(float(temperature))
